...
*Griffe TypeDoc Insiders* is a compatible drop-in replacement for *Griffe TypeDoc*,
...
*Griffe TypeDoc Insiders* can be installed with `pip` [using SSH][using ssh]:
...
Self-hosting the Insiders package makes it possible to depend on *Griffe TypeDoc* normally,
...
with [Twine]:
  ...
  [Github Releases]: https://github.com/pawamoy-insiders/griffe-typedoc/releases
...
>
> Now when running `pip install griffe-typedoc`,
...
Of course, you can use *Griffe TypeDoc Insiders* directly from `git`:
...
When upgrading Insiders, you should always check the version of *Griffe TypeDoc*
