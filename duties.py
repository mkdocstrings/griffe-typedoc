...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("griffe_typedoc", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc griffe_typedoc",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/griffe-typedoc" in origin:
            ctx.run("git remote add upstream git@github.com:mkdocstrings/griffe-typedoc", silent=True, nofail=True)
            ctx.run(
